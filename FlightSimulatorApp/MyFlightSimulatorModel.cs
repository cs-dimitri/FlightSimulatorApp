...
namespace HomeWork
{
    public class MyFlightSimulatorModel : IFlightSimulatorModel
    {
        ...
        public void start()
        {
            ...
            new Thread(delegate () {
                ...
                while (!stop)
                {
                    ...
                    num = rnd.Next(1000);
                    foo = num % 90;
                    string latitude = "set /instrumentation/gps/indicated-vertical-speed" + " " + foo.ToString() + "\n";

                    num = rnd.Next(1000);
                    foo = num % 180;
                    string longitude = "set /instrumentation/gps/indicated-vertical-speed" + " " + foo.ToString() + "\n";
                    ...
                    Thread.Sleep(125);
                   // string answer1 = this.telnetClient.read();
        ...
        private void setValues(string[] values)
        {
            ...
            {
                this.Latitude = Double.Parse(values[7]);
            }
            if (values[9] != "ERR") //verticalSpeed
            {
                
                this.Longitude = Double.Parse(values[7]);
            }
        }
    }
}